<x-layouts.app :title="__('Add Inquiry')">
    <div class="max-w-4xl mx-auto py-8" x-data="inquiryWithAjax({
        ...
        <form action="{{ route('inquiries.store') }}" method="POST" class="space-y-6" @submit="onSubmit">
            ...
            {{-- Inquiry Date --}}
            <flux:input name="inquiry_date" :label="__('Inquiry Date')" type="date" value="{{ date('Y-m-d') }}"
                required />
            ...
            <div
                ...
                <div class="text-sm">
                    <flux:label for="is_ecommerce" class="font-medium text-gray-900 dark:text-neutral-100">
                        {{ __('Store Inquiry') }}
                    </flux:label>
            ...
            <div class="relative" @click.outside="openRequirement = false">
                <div @click="openRequirement = !openRequirement"
                    ...
                    <button x-show="selectedRequirementName" type="button" @click.stop="clearRequirement()"
                        class="ml-auto text-gray-400">&times;</button>
                </div>

                <div x-show="openRequirement" x-cloak
                    class="absolute z-10 w-full mt-2 p-2 rounded-lg bg-white dark:bg-neutral-900 border">
                    <input type="text" x-model="requirementSearch" placeholder="Search..."
                        class="w-full p-2 mb-2 border rounded text-sm">
                    <div class="max-h-60 overflow-y-auto">
                        <template x-for="opt in filteredRequirementTypes" :key="opt.id">
                            <div @click="selectRequirement(opt)"
            ...
            <div class="flex items-center gap-2 mb-4">
                <div class="relative flex-1" @click.outside="openCompany = false">
                    <div @click="openCompany = !openCompany"
                        class="flex items-center w-full p-2 border rounded-md cursor-pointer min-h-[40px]">
                    ...
                    <div x-show="openCompany" x-cloak
                        class="absolute z-10 w-full mt-2 p-2 rounded-lg bg-white dark:bg-neutral-900 border">
                        <input type="text" x-model="companySearch" placeholder="Search companies..."
                            class="w-full p-2 mb-2 border rounded text-sm">
                        <div class="max-h-60 overflow-y-auto">
                ...
                <button type="button" @click="openCompanyModal()"
                    class="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm font-medium">+ Add
                    New</button>
            </div>

            {{-- Company Modal --}}
            <div x-show="showCompanyModal" x-cloak
                class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto pt-10">
                ...
                <div class="relative bg-white dark:bg-neutral-900 rounded-lg p-6 w-full max-w-md" @click.stop>
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" x-model="newCompany.factory_location" placeholder="Factory Location"
                            class="w-full p-2 border rounded">
                        <input type="text" x-model="newCompany.office_location" placeholder="Office Location"
                            class="w-full p-2 border rounded">
                        ...
                        <div class="relative" @click.outside="openIndustry = false">
                            <div @click="openIndustry = !openIndustry"
                                ...
                                <button x-show="selectedIndustryName" type="button" @click.stop="clearIndustry()"
                                    class="ml-auto text-gray-400">&times;</button>
                            </div>

                            <div x-show="openIndustry" x-cloak
                                class="absolute z-50 w-full mt-1 p-3 rounded-lg bg-white dark:bg-neutral-900 border shadow-md">
                                <input type="text" x-model="industrySearch" placeholder="Search industry..."
                                    class="w-full p-2 mb-2 border rounded text-sm">
                                ...
                                <div class="mt-3 border-t pt-3">
                                    <input type="text" x-model="newIndustryName" placeholder="New industry name"
                                    ...
                                    <button type="button"
                                        ...
                                        @click="saveNewIndustryAjax()" :disabled="addingIndustry">
            ...
            <div class="flex items-center gap-2 mb-4">
                <div class="relative flex-1" @click.outside="openCustomer = false">
                    <div @click="openCustomer = !openCustomer"
                        class="flex items-center w-full p-2 border rounded-md cursor-pointer min-h-[40px]">
                ...
                <div class="relative bg-white dark:bg-neutral-900 rounded-lg p-6 w-full max-w-md" @click.stop>
                    ...
                    <div class="space-y-2">
                        <div class="relative" @click.outside="openCompanyForCustomer = false">
                            <div @click="openCompanyForCustomer = !openCompanyForCustomer"
                                class="flex items-center w-full p-2 border rounded-md cursor-pointer min-h-[40px]">
                                <span x-text="selectedCustomerCompanyName || 'Select Company'"></span>
                                <button x-show="selectedCustomerCompanyName" type="button"
                                    @click.stop="clearCompanyForCustomer()"
                                    class="ml-auto text-gray-400">&times;</button>
                            </div>

                            <div x-show="openCompanyForCustomer" x-cloak
                                class="absolute z-50 w-full mt-1 p-2 rounded-lg bg-white dark:bg-neutral-900 border">
                                <input type="text" x-model="companySearchForCustomer" placeholder="Search company..."
                                    class="w-full p-2 mb-2 border rounded text-sm">
                                <div class="max-h-60 overflow-y-auto">
                                    <template x-for="c in filteredCompanyListForCustomer" :key="c.id">
                                        <div @click="selectCompanyForCustomer(c)"
                                            class="p-2 cursor-pointer rounded-md hover:bg-gray-100 dark:hover:bg-neutral-800"
                                            x-text="c.name"></div>
                                    </template>
                                </div>
                        ...
                        <input type="text" x-model="newCustomer.name" placeholder="Full name"
                            class="w-full p-2 border rounded">
                        <input type="email" x-model="newCustomer.email" placeholder="Email"
                            class="w-full p-2 border rounded">
                        <input type="text" x-model="newCustomer.phone" placeholder="Phone"
                            class="w-full p-2 border rounded">
                        <input type="text" x-model="newCustomer.position" placeholder="Position"
                            class="w-full p-2 border rounded">
                        <textarea x-model="newCustomer.notes" rows="2" placeholder="Notes"
            ...
            <select name="process_level" x-model="selectedProcessLevel"
                ...
                <template x-for="lvl in processLevels" :key="lvl">
                    <option :value="lvl" x-text="lvl" :selected="lvl === selectedProcessLevel"></option>
                </template>
            </select>
    ...
    <script>
        function inquiryWithAjax(cfg) {
            return {
                ...
                // new models
                newCompany: { name: '', industry_id: '', factory_location: '', office_location: '' },
                ...
                // modals
                openCompanyModal() { this.showCompanyModal = true; this.companyAjaxError = ''; this.newCompany = { name: '', industry_id: '', factory_location: '', office_location: '' }; this.selectedIndustryName = null; this.newIndustryName = ''; },
                    ...
                    try {
                        ...
                        const res = await fetch("{{ route('companies.ajaxStore') }}", {
                            ...
                            body: JSON.stringify({
                                name,
                                industry_id,
                                factory_location: this.newCompany.factory_location,
                                office_location: this.newCompany.office_location
                            })
                        });
